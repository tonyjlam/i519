...
def perm_gen(kmer):
#'''makes a list with all possible permutations with a length of specified kmer using the nucleotides A, T, G, C'''
...
#'''adds possible permutations generated by perm_gen into kmer_count dictionary'''
    for i in perm_gen(2):
	kmer_count[i] = 1
	...
	while n <= (len(sequence) - 2):
		current = sequence[n:n+2]
		kmer_count[current] += 1
		n += 1
